...
const GlobalContextProvider = ({ children }) => {
  ...
  const obtenerTags = async () => {
    ...
    const response = await fetch(
      "https://my-json-server.typicode.com/IvandevI9/api_tags_spaceappv2/Tags",
    );
  ...
  const filtrarVideosPorTag = () => {
    const videosFiltrados = state.fotosDeGaleria.filter(
      (foto) => foto.tagId === tagSeleccionado,
    );
  ...
  useEffect(() => {
    const getData = async () => {
      const res = await fetch(
        "https://my-json-server.typicode.com/Ivanmx19/api_fotos_spaceapp2/fotos",
      );
